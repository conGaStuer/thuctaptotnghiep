...
<div class="guest">
    ...
    <form action="" method="post">
        <div>
            <b>Tìm theo mã</b>
            <input name="searchKH" type="radio" class="option" value="0" onclick="chosseSearch_KH();" <?php if (!isset($search_KH_by_Name))
                echo "checked"; ?>>
        </div>
        <div>
            <b>Tìm theo tên</b>
            <input name="searchKH" type="radio" class="option" value="1" onclick="chosseSearch_KH();" <?php if (isset($search_KH_by_Name))
                echo "checked"; ?>>
        </div>
...
if (isset($search_KH)) {
    if ($search_KH && !empty($search_KH)) {
        echo '<span class="span">Tìm thấy khách hàng có mã: ' . $makh . '</span>';
        ...
        echo '<span class="span">Không tìm thấy khách hàng có mã ' . $makh . ' trong CSDL</span>';
    }
...
if (isset($search_KH_by_Name)) {
    if ($search_KH_by_Name && !empty($search_KH_by_Name)) {
        echo '<span class="span">Tìm thấy các khách hàng có tên: ' . $tenkh . '</span>';
        echo ' <table>
                <tr>
                    <th>Tên KH</th>
                    <th>Địa chỉ</th>
        ...
        foreach ($search_KH_by_Name['khachhang'] as $rows) {
            echo "<tr>
                    ...
                    <td>" . $rows['dt'] . "</td>
                    <td>" . $rows['cccd'] . "</td>
                    <td><button id='button_" . $rows['makh'] . "' onclick=\"showDienKe(" . $rows['makh'] . ")\" class='btn'>Xem</button></td>
                    </tr>";
            echo "<tr id='dienke_row_" . $rows['makh'] . "' style='display: none;'>
                    <td colspan='6'>
                        <div id='dienke_container_" . $rows['makh'] . "'>";
        ...
        echo '<span class="span">Không tìm thấy khách hàng có tên ' . $tenkh . ' trong CSDL</span>';
    }
